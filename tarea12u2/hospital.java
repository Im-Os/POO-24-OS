...
public class Hospital {
    ...
    public void registrarConsulta(Consulta consulta) {
        if (consulta.getFechaHora() == null || consulta.getConsultorio() == null || consulta.getMedico() == null) {
            System.out.println("La consulta o alguno de sus datos está incompleta.");
            return;
        }
    ...
    public String generarIdPaciente() {
        ...
        int numeroAleatorio = random.nextInt(100000) + 1; // Corrección del rango

        return String.format("P%d%d%d%d", anoActual, mesActual, longitudPacientesMasUno, numeroAleatorio);
    }

    public boolean mostrarPacientePorId(String id) {
        Optional<Paciente> pacienteEncontrado = this.listaPacientes.stream()
                .filter(paciente -> paciente.getId().equals(id)) // Corrección en la comparación
                .findFirst();
    ...
    public String generarIdMedico(String apellido, LocalDate anoNacimiento) {
        Random random = new Random();
        LocalDate fecha = LocalDate.now();
        int anoActual = fecha.getYear();
        int ultimoDigitoAnoNacimiento = anoNacimiento.getYear() % 10; // Obtener el año correctamente
    ...
    public String generarIdConsultorio() {
        ...
        int anoActual = fecha.getYear();
        int numeroAleatorio = random.nextInt(500000) + 1; // Corrección del rango
    ...
    public void mostrarMedicoPorId(String id) {
        Optional<Medico> medicoEncontrado = this.listaMedicos.stream()
                .filter(medico -> medico.getId().equals(id)) // Corrección en la comparación
                .findFirst();
    ...
    public void mostrarConsultorioPorId(String id) {
        Optional<Consultorio> consultorioEncontrado = this.listaConsultorios.stream()
                .filter(consultorio -> consultorio.getId().equals(id)) // Corrección en la comparación
                .findFirst();
    ...
    public boolean validarTelefonoPaciente(String telefonoPaciente) {
        return false;
    }
